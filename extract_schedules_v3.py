...
def extract_lesson(cell_text):
    ...
    if any(marker in lines[0] for marker in ['HJ', 'Periode', 'SJ']):
        return None
    ...
    for line in lines[1:]:
        ...
        if not line:
            continue
        # Skip footer markers
        if any(marker in line for marker in ['HJ', 'Periode', 'SJ', 'Februar']):
            continue
        ...
        elif re.match(r'^(\d+|[A-Z]+\d*|EBad\d|WB\d|MUSIK|NWT\d|PHHS|BIOPR)$', line):
            room = line
...
def parse_page(page):
    ...
    for i, line in enumerate(lines):
        # Look for timestamp pattern (date time format)
    ...
    for row_idx in range(1, len(table)):
        ...
        for day_idx in range(5):
            ...
            if lesson_data and lesson_data['subject']:
                # Skip footer-like entries
                if re.match(r'^\d+\.', lesson_data['subject']):
                    continue
                lessons.append({
                    'period': period,
                    'dayIndex': day_idx,
